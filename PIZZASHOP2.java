import java.util.Scanner;
import java.util.Vector;

public class PizzaShop {

    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in);

        // Greet the customer
        System.out.println("Welcome to the Pizza Shop! 🍕");

        // Ask for customer's name
        System.out.print("Please enter your name: ");
        String customerName = scanner.nextLine();

        // Validate phone number to ensure it has exactly 10 digits
        String phoneNumber = "";
        while (true) {
            System.out.print("Please enter your 10-digit phone number: ");
            phoneNumber = scanner.nextLine();
            if (phoneNumber.matches("\\d{10}")) {
                break;
            } else {
                System.out.println("Invalid phone number! Please enter exactly 10 digits.");
            }
        }

        // Pizza sizes and their prices (using Vector instead of array)
        Vector<String> sizes = new Vector<>();
        sizes.add("Small");
        sizes.add("Medium");
        sizes.add("Large");

        Vector<Double> sizePrices = new Vector<>();
        sizePrices.add(200.0);  // Small price
        sizePrices.add(300.0);  // Medium price
        sizePrices.add(400.0);  // Large price

        // Toppings and their prices (using Vector instead of array)
        Vector<String> toppings = new Vector<>();
        toppings.add("Cheese");
        toppings.add("Olives");
        toppings.add("Mushrooms");
        toppings.add("Peppers");
        toppings.add("Bacon");
        toppings.add("Pineapple");
        toppings.add("Onions");
        toppings.add("Tomatoes");
        toppings.add("Chicken");
        toppings.add("Spinach");

        Vector<Double> toppingPrices = new Vector<>();
        toppingPrices.add(30.0);
        toppingPrices.add(40.0);
        toppingPrices.add(50.0);
        toppingPrices.add(60.0);
        toppingPrices.add(70.0);
        toppingPrices.add(40.0);
        toppingPrices.add(50.0);
        toppingPrices.add(30.0);
        toppingPrices.add(80.0);
        toppingPrices.add(40.0);

        // Ask for pizza size with validation
        int sizeChoice = 0;
        while (true) {
            System.out.println("\nChoose a pizza size:");
            for (int i = 0; i < sizes.size(); i++) {
                System.out.println((i + 1) + ". " + sizes.get(i) + " - RS. " + sizePrices.get(i));
            }
            System.out.print("Please enter the number corresponding to your pizza size (1-3): ");
            sizeChoice = scanner.nextInt();

            if (sizeChoice >= 1 && sizeChoice <= 3) {
                break;  // Valid input, exit the loop
            } else {
                System.out.println("Invalid choice! Please select a number between 1 and 3.");
            }
        }

        // Get the price of the chosen pizza size
        double totalPrice = sizePrices.get(sizeChoice - 1);

        // Ask for toppings with validation
        System.out.println("\nSelect toppings (Choose 0 to stop):");
        for (int i = 0; i < toppings.size(); i++) {
            System.out.println((i + 1) + ". " + toppings.get(i) + " - RS. " + toppingPrices.get(i));
        }

        double toppingCost = 0.0;
        StringBuilder selectedToppings = new StringBuilder();
        while (true) {
            System.out.print("Enter the topping number you want to add (0 to finish): ");
            int toppingChoice = scanner.nextInt();
            
            if (toppingChoice == 0) {
                break;  // Stop adding toppings
            } else if (toppingChoice > 0 && toppingChoice <= toppings.size()) {
                toppingCost += toppingPrices.get(toppingChoice - 1);
                selectedToppings.append(toppings.get(toppingChoice - 1)).append(" ");
            } else {
                System.out.println("Invalid topping choice! Please select a valid topping number between 1 and 10.");
            }
        }

        // Calculate the total price
        totalPrice += toppingCost;

        // Ask for order confirmation
        scanner.nextLine(); // Consume newline character
        System.out.println("\nYour Order Summary:");
        System.out.println("Customer Name: " + customerName);
        System.out.println("Phone Number: " + phoneNumber);
        System.out.println("Pizza Size: " + sizes.get(sizeChoice - 1));
        System.out.println("Toppings: " + (selectedToppings.length() > 0 ? selectedToppings.toString() : "None"));
        System.out.println("Total Price: RS. " + totalPrice);

        // Confirm the order
        System.out.print("Do you want to confirm your order? (Yes/No): ");
        String confirmation = scanner.nextLine().toLowerCase();

        if (confirmation.equals("yes")) {
            System.out.println("\nThank you for your order! Your pizza will be ready soon! 🍕");
        } else {
            System.out.println("Your order has been canceled.");
        }

        // Close the scanner to avoid resource leaks
        scanner.close();
    }
}